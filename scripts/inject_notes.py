...
def gh_get(path: str) -> dict | list:
    ...
    try:
        ...
        print(
            f"[inject-notes] GitHub API error {e.code} for {url}: {body}",
            file=sys.stderr,
        )
        sys.exit(1)
...
def inject(readme_path: str, content: str) -> bool:
    ...
    if updated == original:
        print(
            "[inject-notes] README unchanged — notes are up to date.", file=sys.stderr
        )
        return False
    ...
    print(
        f"[inject-notes] Injected {len(content.splitlines())} line(s) into {readme_path}",
        file=sys.stderr,
    )
    return True
...
def main():
    if not GITHUB_TOKEN:
        print(
            "[inject-notes] Warning: GITHUB_TOKEN not set. Unauthenticated rate limits apply.",
            file=sys.stderr,
        )
