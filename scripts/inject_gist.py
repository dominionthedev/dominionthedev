...
USERNAME = (
    os.environ.get("USERNAME") or os.environ.get("GITHUB_ACTOR") or "dominionthedev"
)
...
def inject(readme_path: str, content: str) -> bool:
    ...
    print(
        f"[inject-gists] Injected {len(content.splitlines())} line(s)", file=sys.stderr
    )
    return True
...
def main():
    if not GITHUB_TOKEN:
        print(
            "[inject-gists] Warning: No GITHUB_TOKEN — rate limits apply.",
            file=sys.stderr,
        )
